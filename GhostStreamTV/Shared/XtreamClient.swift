...
struct XtreamClient {
    ...
    func seriesInfo(seriesId: Int) async throws -> [Episode] {
        let result = try await fetchResolved(XtreamSeriesInfo.self,
        ...
        for (seasonKey, list) in info.episodesBySeason {
            ...
            for ep in list {
                ...
                episodes.append(Episode(
                    ...
                    containerExtension: ext,
                    directSource: direct,
                    image: resolveAssetURL(ep.image, base: result.base),
...
struct XtreamSeriesInfo: Decodable {
    struct EpisodeEntry: Decodable {
        struct EpisodeInfo: Decodable {
            ...
            init(from decoder: Decoder) throws {
                let c = try decoder.container(keyedBy: CodingKeys.self)
                image = c.flexibleString(.movieImage) ?? c.flexibleString(.coverBig) ?? c.flexibleString(.streamIcon)
                duration = c.flexibleString(.duration)
                plot = c.flexibleString(.plot)
            }
        }
        ...
        init(from decoder: Decoder) throws {
            ...
            let info = try? c.decode(EpisodeInfo.self, forKey: .info)
            image = info?.image ?? c.flexibleString(.movieImage)
            duration = info?.duration ?? c.flexibleString(.duration)
            plot = info?.plot ?? c.flexibleString(.plot)
        }
    }
